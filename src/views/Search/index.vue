    ...
    <div class="search-main">
      ...
      <template v-if="!searchVal">
        <section class="search-recommend">
          ...
          <div class="recommend-content font-28">
            <span class="recommend-item" v-for="item in hotSearch" :key = 'item.id' @click = 'saveSearch(item.keyword)'>{{ item.keyword }}</span>
          </div>
        </section>
        <!-- 热门搜索 end -->

        <!-- 最近搜索 begin -->
        <section class="search-history" v-show="historyList.length > 0">
          <div class="title font-26">
          ...
          <div class="history-content font-28">
            <span class="history-item" v-for="item in historyList" :key="item" @click="saveSearch(item)">
      ...
      <template v-else>
        <section class="search-content">
          <p class="item font-28" v-for="item in searchList" :key = "item" @click = 'saveSearch(item)'>{{ item }}</p>
        </section>
      </template>
...
<script>
import { getHotSearch, getSearchList } from '@/api/cartoon'
export default {
  name: 'Search',
  data () {
    return {
  ...
  methods: {
    ...
    getHistory () {
      this.historyList = window.localStorage.getItem('search') ? JSON.parse(window.localStorage.getItem('search')) : []
    },
    delHistory () {
      this.historyList = []
